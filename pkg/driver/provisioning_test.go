...
func TestParseControllerConfig(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			volumeName:          "pvc-123",
			params:              map[string]string{"volumeRootPath": "/tempZone/home/alice/"},
		...
		},
		{
			...
			volumeName:          "pvc-123",
			params:              map[string]string{"volumeRootPath": "/tempZone/home/alice", "noVolumeDir": "true"},
			wantRoot:            "/tempZone/home/alice",
		...
		},
		{
			...
			volumeName:    "pvc-123",
			params:        map[string]string{"volumeRootPath": "relative"},
		...
		},
		{
			...
			volumeName:    "pvc-123",
			params:        map[string]string{"volumeRootPath": "/tempZone/home/alice", "noVolumeDir": "sometimes"},
			wantErrorCode: codes.InvalidArgument,
		},
	}
...
func TestDynamicVolumeProvisioningMode(t *testing.T) {
	if !isDynamicVolumeProvisioningMode(map[string]string{"provisioningMode": " DYNAMIC "}) {
		t.Fatal("dynamic provisioning mode was not recognized")
	}
	if isDynamicVolumeProvisioningMode(map[string]string{"provisioningMode": "static"}) {
...
func TestValidateVolumeCapabilitiesValidatesAndConfirmsConfiguration(t *testing.T) {
	driver := &Driver{
		config:  &commons.Config{},
		secrets: map[string]string{},
	}
	volumeContext := map[string]string{"path": "/volume"}
	...
	if response.GetConfirmed() == nil {
		t.Fatal("ValidateVolumeCapabilities() did not confirm a valid configuration")
	}
	if response.GetConfirmed().GetVolumeContext()["path"] != "/volume" {
		t.Fatalf("VolumeContext = %#v", response.GetConfirmed().GetVolumeContext())
	}
	if response.GetConfirmed().GetParameters()["path"] != "/export" {
		t.Fatalf("Parameters = %#v", response.GetConfirmed().GetParameters())
	}
}

func TestValidateVolumeCapabilitiesRejectsInvalidConfiguration(t *testing.T) {
	driver := &Driver{config: &commons.Config{}, secrets: map[string]string{}}
	_, err := driver.ValidateVolumeCapabilities(context.Background(), &csi.ValidateVolumeCapabilitiesRequest{
		VolumeId:           "static-volume",
		Parameters:         map[string]string{"client": "unsupported"},
		VolumeCapabilities: []*csi.VolumeCapability{{AccessMode: &csi.VolumeCapability_AccessMode{Mode: csi.VolumeCapability_AccessMode_MULTI_NODE_MULTI_WRITER}}},
	})
